        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <h1 className="mt-2 text-4xl font-black text-brand-ink">
              Welcome back to FridgeSmart
            </h1>
            <p className="mt-3 max-w-2xl text-base leading-7 text-brand-ink/75">
              This is where logged-in users should quickly see what they have, what is expiring, and what they can cook next.
            </p>
          </div>
          <div className="rounded-full bg-[#FFF5E8] px-4 py-2 text-sm font-black text-brand-peach">
            ...
            {recipeCards.map((recipe) => (
              <div key={recipe.title} className="rounded-[1.5rem] bg-white p-5 shadow-card">
                <div className="flex items-center justify-between gap-4">
                  ...
                  <div className="rounded-full bg-brand-softMint px-3 py-1 text-xs font-black text-brand-olive">
                    {recipe.ready}
                  </div>
                </div>
              </div>
          ...
          <div className="mt-5 space-y-4">
            {expiringItems.map((item) => (
              <div key={item.name} className="rounded-[1.5rem] bg-white p-5 shadow-card">
                ...
                <div className="mt-3 inline-flex rounded-full bg-[#FFF5E8] px-3 py-1 text-xs font-black text-brand-peach">
                  {item.window}
                </div>
              </div>
            ))}
